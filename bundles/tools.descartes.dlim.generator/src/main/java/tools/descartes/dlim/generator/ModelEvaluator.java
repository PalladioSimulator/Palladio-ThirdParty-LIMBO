...
public class ModelEvaluator {
     ...
     * @param noiseMode
     *            set this to IGeneratorConstants.EVALUATION for normal model evaluation, set it to
     *            IGeneratorConstants.CALIBRATION, if Noises are to always return 0.
     */
    ...
    /**
     * Creates a default ModelEvaluator for the given Descartes Load Intensity Model. The default
     * ModelEvaluator evaluates Noises with IGeneratorContstants.DEFAULTRANDOMSEED.
     *
     * @param rootSequence
     *            The root model element. This is always a Sequence.
     */
    ...
    /**
     * Evaluates the model for the specified time interval and stores the result in a list. Uses
     * multithreading for performance.
     * 
     * @param startTime
     *            The time to start evaluation (inclusive).
     * @param endTime
     *            The time to end evaluation (exclusive).
     * @param step
     *            The sampling steps.
    ...
    /**
     * Gets the Delta between the root function and the final output including the index-th
     * combinators.
     * 
     * @param rootTime
     *            time stamp as defined at this moment within the root sequence
     * @param index
     *            combinator's index
     * @param wasMult
     *            writes back if the combinator in this function was multiplicative or not
    ...
    public double getArrivalRateDelta(double rootTime, int index, boolean[] wasMult) {
        try {
            ...
            ArrayList<Combinator> combList = new ArrayList<>();
    ...
    /**
     * Returns a model duration that guarantees model execution. Returns the model duration if it is
     * not infinite. Returns the duration of a single iteration of the loop sequence, otherwise.
     * 
    ...
    /**
     * The random number generator. This generator has already been initialized using the correct
     * seed.
     *
    ...
    /*
     * Gets the value of the Function f and all child Functions, which are contained in f's child
     * Combinators.
     */
    ...
    /*
     * Get function type and proceed accordingly. Simple Functions are passed to the
     * FunctionValueCalculator, since they do not need to keep the rootTime and loopTime any longer.
     */
    private double calculateFunctionValue(Function f, double x, double rootTime, double loopTime) {
        if (f instanceof Sequence) {
            ...
            return Math
                .abs(getFunctionNodeArrivalRate(((AbsoluteValueFunction) f).getFunction(), x, rootTime, loopTime));
        }
        return calculator.getSimpleFunctionValue(f, x);
    }

    /*
     * Calculate the arrival rate of the currently running Element within the Sequence. Return 0 if
     * Sequence is currently inactive. Also, convert times.
     */
    private double calculateSequenceValue(Sequence seq, double x, double rootTime, double loopTime) {
        ...
        if (loopTime < seq.getFirstIterationStart() || loopTime >= seq.getFirstIterationEnd()) {
            if (ModelEvaluatorUtil.getParentCombinator(seq) != null) {
                return ModelEvaluatorUtil.neutralElement(ModelEvaluatorUtil.getParentCombinator(seq)
                    .getOperator());
            } else {
                return 0.0;
            }
    ...
    /*
     * Calculate the arrival rate of the FunctionNode held by the Element. Return 0 if the Element
     * is currently inactive. Convert times, depending on referenceClockType and
     * pointOfReferenceClockObject.
     */
    private double getElementArrivalRateAtTime(TimeDependentFunctionContainer e, double rootTime, double guardTime,
            double loopTime, double seqTime) {
        // time guard
        ...
        if (e.getPointOfReferenceClockType()
            .toString()
            .contains("CONTAINERCLOCK")) {
            x = guardTime - e.getFirstIterationStart();
        } else if (e.getPointOfReferenceClockType()
            .toString()
            .contains("SEQCLOCK")) {
            if (e.getPointOfReferenceClockObject() == null) {
                x = seqTime;
            } else {
                x = e.getPointOfReferenceClockObject()
                    .getSeqTime();
            }
        } else if (e.getPointOfReferenceClockType()
            .toString()
        ...
        if (e.getFunction() == null) {
            ...
            if (ModelEvaluatorUtil.getParentCombinator(ModelEvaluatorUtil.getParentSequence(e)) != null) {
                functionValue = ModelEvaluatorUtil
                    .neutralElement(ModelEvaluatorUtil.getParentCombinator(ModelEvaluatorUtil.getParentSequence(e))
                        .getOperator());
            }
        } else {
     ...
     * 
     * @return True, if at least one Sequence in the model contains a ReferenceClockObject.
     */
