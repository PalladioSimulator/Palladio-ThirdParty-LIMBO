...
public class DiffAnalyzer {
    ...
    /**
     * Create a new DiffAnalyzer for the model that is being evaluated by the evaluator. All output
     * will be written into the Eclipse project, described by its root path (projectPath).
     *
     * @param evaluator
     *            the model's evaluator
     * @param projectPath
     ...
     *            The arrival rates as read from the original trace. The file to which the model is
     *            to be compared.
     * @param offset
     *            the offset of the first arrival rate tuple in the arrival rate file
     * @return A list of difference metrices: 1. absolute mean, 2. absolute median, 3. DTW based
     *         difference, 4. relative mean, 5. relative median
     */
    public List<Double> calculateDiff(List<ArrivalRateTuple> readArrivalRates, double offset) {
        try {
            ...
            for (ArrivalRateTuple t : readArrivalRates) {
                ...
                if (modelTimeStamp > 0 && modelTimeStamp < evaluator.getTerminatingDuration()) {
            ...
            double dtwDist = FastDTW.getWarpInfoBetween(fileTS, modelTS, 10, new EuclideanDistance())
                .getDistance();
