...
public class FunctionValueCalculator {
    ...
    private double getBurstValue(Burst f, double x) {
        if (f instanceof LinearIncreaseAndDecline) {
            ...
            return calculateExponentialIncreaseAndDeclineValue(x, burst.getBase(), burst.getPeak(), burst.getPeakTime(),
                    ModelEvaluatorUtil.getFunctionDuration(f));
    ...
    private double getTrendValue(Trend f, double x) {
        if (f instanceof LinearTrend) {
            ...
            return calculateExponentialTrendValue(x, trend.getFunctionOutputAtEnd(), trend.getFunctionOutputAtStart(),
                    ModelEvaluatorUtil.getFunctionDuration(f));
        } else if (f instanceof LogarithmicTrend) {
            LogarithmicTrend trend = (LogarithmicTrend) f;
            return calculateLogarithmicTrendValue(x, trend.getFunctionOutputAtEnd(), trend.getFunctionOutputAtStart(),
                    trend.getOrder(), ModelEvaluatorUtil.getFunctionDuration(f));
    ...
    private double calculateLogarithmicTrendValue(double x, double endValue, double startValue, double order,
        ...
        if (startValue > endValue) {
            ...
            return endValue
                    + (startValue - endValue) * (1 / order) * Math.log((tmpX * (Math.exp(order) - 1) / duration) + 1);
        }
        return startValue
    ...
    private double calculateLinearIncreaseAndDeclineValue(double x, double baseValue, double peakValue, double peakTime,
            double duration) {
        double tmpX = x;
        if (x > peakTime) {
