...
/**
 * Utility class containing functionality needed for the extraction of basic Trends. For more
 * advanced trend extraction, see {@link RegressiveTrendExtractor}.
 * 
...
public final class BasicTrendExtractionUtilities {
     ...
     * 
     * if calibrateTrendPoints==true: Trend points in the container must indicate the target arrival
     * rate that must be reached at a trend segment's end/beginning.
     * 
     * @param root
     *            The model's root sequence.
     * @param container
     *            The container containing all known extraction data.
     * @param trendOffset
     *            The time offset for the beginning of the first trend segment. Usually corresponds
     *            to the largest peak in the seasonal iterations.
     * @param calibrateTrendPoints
     *            Indicates whether trend points are arrival rate targets, which then require
     *            additional calibration, or if they are already the final values to be used in
     *            trend building.
     */
    public static void buildTrendPart(Sequence root, ExtractionDataContainer container, double trendOffset,
            boolean calibrateTrendPoints) {
        ...
        if (trendDuration > 0.0 && container.getMaxPeakOffset() + trendDuration < container.getDuration()
            ...
            trendSequence.getSequenceFunctionContainers()
                .add(offsetElement);
            ...
            for (int i = 0; i < container.getTrendPointValues().length - 1; i++) {
                ...
                trendElement.setFunction(createTrendWithCorrectShape(container.getSeasonalShape()));
                trendSequence.getSequenceFunctionContainers()
                    .add(trendElement);
            }
            TimeDependentFunctionContainer holdElement = factory.createTimeDependentFunctionContainer();
            holdElement.setDuration(container.getDuration() - trendOffset
                    - trendDuration * (container.getTrendPointValues().length - 1));
            ...
            for (TimeDependentFunctionContainer e : trendSequence.getSequenceFunctionContainers()) {
                ...
                if (f instanceof Trend) {
                    ...
                    if (calibrateTrendPoints) {
                        trend.setFunctionOutputAtStart(
                                calibrateTrendStartValue(root, trend, container.getTrendPointValues()[i]));
                    } else {
                        trend.setFunctionOutputAtStart(container.getTrendPointValues()[i]);
                    }
                    i++;
                    if (calibrateTrendPoints) {
                        trend.setFunctionOutputAtEnd(
                                calibrateTrendEndValue(root, trend, container.getTrendPointValues()[i]));
                    } else {
            ...
            Constant offset1 = (Constant) trendSequence.getSequenceFunctionContainers()
                .get(0)
                .getFunction();
            offset1.setConstant(((Trend) (trendSequence.getSequenceFunctionContainers()
                .get(1)
                .getFunction())).getFunctionOutputAtStart());

            int lastIndex = trendSequence.getSequenceFunctionContainers()
                .size() - 1;
            Constant offset2 = (Constant) trendSequence.getSequenceFunctionContainers()
                .get(lastIndex)
                .getFunction();
            offset2.setConstant(((Trend) (trendSequence.getSequenceFunctionContainers()
                .get(lastIndex - 1)
    ...
    /**
     * Build the Trend part for the periodic extraction process. Trend points in the container must
     * indicate the target arrival rate that must be reached at a trend segment's end/beginning.
     * 
     * @param root
     *            The model's root sequence.
     * @param container
     ...
     *            Do I need this (or remove)??
     * @param trendPoints
     *            I probably should remove this parameter as well.
     */
    public static void buildRepeatingTrend(Sequence root, int[] seasonalsPerTrend, double[] trendPoints,
        ...
        if (trendDuration > 0.0 && trendPoints.length > 1) {
            ...
            offsetElement.setName("offset");
            offsetElement.setFunction(factory.createConstant());
            trendSequenceContainer.getSequenceFunctionContainers()
                .add(offsetElement);
            ...
            for (int i = 0; i < trendPoints.length - 1; i++) {
                ...
                trendElement.setName("trend" + i);
                trendElement.setFunction(createTrendWithCorrectShape(container.getTrendShape()));
                trendSequence.getSequenceFunctionContainers()
                    .add(trendElement);
            }
            ...
            Constant offset1 = (Constant) trendSequenceContainer.getSequenceFunctionContainers()
                .get(0)
                .getFunction();
            offset1.setConstant(((Trend) (trendSequence.getSequenceFunctionContainers()
                .get(0)
     ...
     * 
     * @param shapeString
     *            the String literal for the trend's shape.
    ...
    public static Trend createTrendWithCorrectShape(String shapeString) {
        try {
            return (Trend) DlimPackage.eINSTANCE.getDlimFactory()
                .create((EClass) (DlimPackage.eINSTANCE.getEClassifier(shapeString)));
