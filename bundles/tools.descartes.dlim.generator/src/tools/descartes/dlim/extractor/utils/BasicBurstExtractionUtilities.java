...
/**
 * Utility class containing functionality needed for the extraction of basic bursts.
 * 
...
public final class BasicBurstExtractionUtilities {
    ...
    /**
     * Detects a burst if its arrival rate is this greater than the respective (modified) seasonal
     * pattern.
     */
    ...
    public static void buildBurstPart(Sequence root, ExtractionDataContainer container) {
        if (container.getBursts()
            ...
            for (ArrivalRateTuple burst : container.getBursts()) {
                ...
                burstSequence.getSequenceFunctionContainers()
                    .add(offsetContainer);
                burstSequence.getSequenceFunctionContainers()
                    .add(burstContainer);
                index++;
            ...
            for (TimeDependentFunctionContainer tdfc : burstSequence.getSequenceFunctionContainers()) {
                if (tdfc.getFunction() != null && tdfc.getFunction() instanceof Burst) {
                    Burst burstFunction = (Burst) tdfc.getFunction();
                    burstFunction.setPeak(calibrateBurstPeakValue(root, burstFunction, container.getBursts()
                        .get(index)
                        .getArrivalRate()));
                    index++;
                }
    ...
    /**
     * Get the time-stamps and arrival rates of bursts (meaning major deviations from the extracted
     * model instance).
     * 
     * @param root
     *            The model's root sequence.
     * @param peakBaselineModel
     *            The baseline sequence, without local minima. Previously constructed specifically
     *            for burst detection.
     * @param container
     *            The container containing all known extraction data.
     * @return The arrival rate tuples of the burst peaks.
     */
    public static ArrayList<ArrivalRateTuple> getBursts(Sequence root, Sequence peakBaselineModel,
            ExtractionDataContainer container) {
        double timeOffset = container.getLocalMins()
            .get(0)
            .getTimeStamp();
        ...
        for (ArrivalRateTuple max : container.getLocalMaxes()) {
            ...
            if (currentTime >= 0 && max.getArrivalRate() > currentBaselineModel.getArrivalRateAtTime(currentTime)
                ...
                if (burstList.size() > 1) {
                    if (burstList.get(burstList.size() - 1)
                        ...
                        if (burstList.get(burstList.size() - 1)
                            .getArrivalRate() < burstList.get(burstList.size() - 2)
                                .getArrivalRate()) {
    ...
    /**
     * Calculates a static width for all bursts in a trace. Width is based on the trace metadata in
     * container.
     * 
     * @param container
    ...
    public static double calculateBurstWidth(ExtractionDataContainer container) {
        ...
        } else if (container.getSubContainerList() != null && !container.getSubContainerList()
            .isEmpty()) {
            return calculateBurstWidth(container.getSubContainerList()
                .get(0));
        }
