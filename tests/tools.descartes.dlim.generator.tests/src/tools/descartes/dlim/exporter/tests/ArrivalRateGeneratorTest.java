...
/**
 * Generates arrival rates and writes them to a file. Compares written rates with model output.
 * 
...
public class ArrivalRateGeneratorTest extends TestCase {
    ...
    public void testWriteArrivalRates() {
        ...
        ArrivalRateGenerator.writeArrivalRates(ArrivalRateGenerator.generateArrivalRates(evaluator, 1.0),
                new File("").getAbsolutePath(), TEST_NAME, "", "csv");
        IDlimArrivalRateReader reader = new DefaultArrivalRateReader();
        try {
            List<ArrivalRateTuple> list = reader.readFileToList("./arrivalRates/" + TEST_NAME + "ArrivalRates.csv",
                    0.0);
