...
import tools.descartes.dlim.util.DlimUtil;
...
public class TimeStampWriterTest extends TestCase {
    ...
    private static final String TEST_EQUALDISTANCE_NAME = DlimTestUtils.TEST_OUTPUT_DIR + "/equaldisttstest.txt";
    private static final String TEST_RANDOM_NAME = DlimTestUtils.TEST_OUTPUT_DIR + "/randomtstest.txt";
    ...
    public void testEqualDistanceTimeStamps() {
        Sequence root = (Sequence) DlimUtil.getRootEObject(DlimTestUtils.TEST_MODEL);
        ModelEvaluator evaluator = new ModelEvaluator(root);
        EqualDistanceTimestampWriter tsw = new EqualDistanceTimestampWriter(";");
        // Create TimeStamps
        tsw.generateTimeStampsFromArrivalRates(new File(TEST_EQUALDISTANCE_NAME),
                ArrivalRateGenerator.generateArrivalRates(evaluator, 1.0), 4, 1.0, 1.0);
    ...
    /**
     * Test timestamp generation for timestamps with a uniform random distribution.
     */
    public void testUniformRandomTimeStamps() {
        ...
        tsw.generateTimeStampsFromArrivalRates(new File(TEST_RANDOM_NAME),
                ArrivalRateGenerator.generateArrivalRates(evaluator, 1.0), 4, 1.0, 1.0, evaluator.getDuration());

        checkTimeStampCreation(evaluator, TEST_RANDOM_NAME);
    }

    /**
     * Tests for correct time stamp creation for any given time stamp writer implementing the
     * TimeStampWriter Interface.
     * 
     * @param tsOutPath
     * @param tsw
     */
    private void checkTimeStampCreation(ModelEvaluator evaluator, String tsOutPath) {
        // Read time stamps back in for checking
        RequestTimeSeriesReader.createArrivalRatesFromSortedTimeStamps(tsOutPath, DlimTestUtils.TEST_OUTPUT_DIR);
        String tmpArrivalRateFileName = tsOutPath.substring(0, tsOutPath.length() - 4) + "ArrivalRates.txt";
        try {
            List<ArrivalRateTuple> list = ArrivalRateReader.readFileToList(tmpArrivalRateFileName, 0.0);
            assertEquals((int) evaluator.getArrivalRateAtTime(5.5), list.get(5)
                .getArrivalRate(), DlimTestUtils.FP_DELTA);
            assertEquals(7.5, list.get(7)
                .getTimeStamp(), DlimTestUtils.FP_DELTA);
            assertEquals((int) evaluator.getArrivalRateAtTime(2.5), list.get(2)
                .getArrivalRate(), DlimTestUtils.FP_DELTA);
            assertEquals(11.5, list.get(11)
                .getTimeStamp(), DlimTestUtils.FP_DELTA);
        } catch (IOException e) {
